            ...
            ${details.participants.length > 0
              ? `<ul class="participant-list">
                  ${details.participants
                    .map(
                      (email) => `<li class="participant-item"><span class="participant-email">${email}</span><button type="button" class="participant-delete" data-activity="${name}" data-email="${email}" aria-label="Remove participant">&times;</button></li>`
                    )
                    .join("")}
                </ul>`
              : '<p class="participant-list-empty">No participants yet.</p>'}
          </div>
        `;

        activitiesList.appendChild(activityCard);

        // Attach delete handlers to participant buttons
